...
vi.mock("./partner-integration.service", () => ({
  dispatchShipmentToPartner: vi.fn().mockResolvedValue(null),
}));

const { quotePlatformDelivery, acceptDeliveryQuote } =
  await import("./delivery-operations.service");

beforeEach(() => {
  vi.clearAllMocks();
  prismaMock.$transaction.mockImplementation(async (value: unknown) => {
    if (typeof value === "function")
      return (value as (tx: typeof txMock) => unknown)(txMock);
  ...
  txMock.platformShipment.create.mockResolvedValue({
    id: "shipment_1",
    trackingCode: "WSL-1",
  });
...
describe("quotePlatformDelivery", () => {
  it("rejects pickup and completed orders before quoting", async () => {
    prismaMock.order.findUnique.mockResolvedValue({
      ...
      delivery: null,
      branch: null,
      platformShipment: null,
    });
    await expect(quotePlatformDelivery("order_1")).rejects.toThrow(
      BusinessRuleError,
    );
    ...
    await expect(quotePlatformDelivery("order_1")).rejects.toThrow(
      "can no longer request delivery",
    );
  });
  ...
  it("returns one cheapest quote per partner when pricing rules overlap", async () => {
    prismaMock.order.findUnique.mockResolvedValue({
      ...
      status: "NEW",
      platformShipment: null,
      branch: { lat: 15.5, lng: 32.5 },
      delivery: { lat: 15.51, lng: 32.51 },
    });
    ...
    prismaMock.deliveryQuote.create.mockImplementation(
      async ({
        ...
        data: { partnerId: string; fee: number; pricingRuleId: string };
      }) => data,
    );

    const quotes = (await quotePlatformDelivery(
      "order_1",
    )) as unknown as Array<{
      partnerId: string;
      fee: number;
...
describe("acceptDeliveryQuote", () => {
  it("updates order and payment totals and creates COD custody for the exact final total", async () => {
    ...
    expect(txMock.codCollection.create).toHaveBeenCalledWith({
      data: { shipmentId: "shipment_1", expectedAmount: 120, currency: "SDG" },
    });
  });
  ...
  it("rejects duplicate shipment creation", async () => {
    txMock.deliveryQuote.findFirst.mockResolvedValue({
      ...
      order: {
        paymentMethod: "CASH",
        payment: null,
        platformShipment: { id: "existing" },
      },
    });
    await expect(acceptDeliveryQuote("order_1", "quote_1")).rejects.toThrow(
      "already exists",
    );
  });
});
