...
import {
  ...
  issueStoreAffiliatePortalAccessAction,
...
export default async function StoreAffiliatesPage({ searchParams }: { searchParams: Promise<{ portalToken?: string }> }) {
  ...
  const portalLink = query.portalToken && /^[A-Za-z0-9_-]{32,100}$/.test(query.portalToken)
    ? `${origin}/affiliate/portal/${query.portalToken}`
    : null;
  ...
  return <div className="mx-auto max-w-7xl space-y-7" dir="rtl">
    ...
    {portalLink && <section className="rounded-2xl border border-emerald-300 bg-emerald-50 p-5 text-emerald-950"><h2 className="font-black">رابط بوابة المسوّق جاهز</h2><p className="mt-1 text-sm">أرسله للمسوّق المعني فقط. صالح لمدة 30 يوماً، وإصدار رابط جديد يلغي السابق.</p><code dir="ltr" className="mt-3 block break-all rounded-lg bg-white p-3 text-xs select-all">{portalLink}</code></section>}
    ...
    <section><h2 className="mb-3 font-black">المسوّقون ({affiliates.length})</h2><div className="grid gap-4 lg:grid-cols-2">{affiliates.map(affiliate => {
      ...
      return <article key={affiliate.id} className="rounded-2xl border bg-[var(--card)] p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{affiliate.name}</h3><p dir="ltr" className="mt-1 text-right text-sm text-[var(--muted-foreground)]">{affiliate.phone}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${affiliate.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`}>{affiliate.status === 'ACTIVE' ? 'نشط' : 'موقوف'}</span></div><code dir="ltr" className="mt-4 block break-all rounded-lg bg-[var(--muted)] p-3 text-xs select-all">{link}</code><div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-[var(--muted-foreground)]"><span>{affiliate._count.visits} زيارة</span><span>{affiliate._count.attributions} طلب منسوب</span><span>{affiliate._count.commissions} عمولة</span>{canConfigure && affiliate.status === 'ACTIVE' && <form action={issueStoreAffiliatePortalAccessAction}><input type="hidden" name="affiliateId" value={affiliate.id}/><button className="rounded-lg border border-[var(--primary)] px-3 py-1.5 font-bold text-[var(--primary)]">إصدار رابط البوابة</button></form>}{canConfigure && <form action={setStoreAffiliateStatusAction} className="ms-auto"><input type="hidden" name="affiliateId" value={affiliate.id}/><input type="hidden" name="status" value={affiliate.status === 'ACTIVE' ? 'SUSPENDED' : 'ACTIVE'}/><button className="rounded-lg border px-3 py-1.5 font-bold text-[var(--foreground)]">{affiliate.status === 'ACTIVE' ? 'إيقاف' : 'إعادة التفعيل'}</button></form>}</div></article>;
