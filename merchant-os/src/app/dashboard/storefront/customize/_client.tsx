...
export function CustomizeClient({ settings, slug, merchantName, initialLogo }: { settings: Settings; slug: string; merchantName: string; initialLogo: string }) {
  ...
  const rtl = dir === 'rtl';
  const copy = rtl ? {
    builder: 'منشئ واجهة المتجر', sections: 'أقسام المتجر', canvas: 'المعاينة المباشرة', settings: 'خصائص المتجر', drag: 'اسحب لترتيب الأقسام', announcement: 'شريط الإعلان', hero: 'الواجهة الرئيسية', trust: 'مزايا المتجر', categories: 'التصنيفات', featured: 'منتجات مميزة', products: 'كل المنتجات', testimonials: 'آراء العملاء', social: 'تابعنا وتواصل', visible: 'ظاهر', hidden: 'مخفي', logo: 'شعار المتجر', banner: 'بانر المتجر', uploadError: 'تعذر رفع الصورة', unsaved: 'لديك تعديلات غير محفوظة', mobile: 'جوال', desktop: 'سطح المكتب', sampleCategory: 'منتجات جديدة', remove: 'حذف', upload: 'اسحب الصورة هنا أو اضغط للرفع', uploading: 'جاري رفع الصورة…', open: 'مفتوح', closed: 'مغلق', shopNow: 'تسوق الآن', all: 'الكل', offers: 'العروض', popular: 'الأكثر طلباً', bannerHint: 'يفضل مقاس 1600 × 600', templates: 'قوالب جاهزة', templatesHint: 'اختر قالباً ثم عدّل كل تفاصيله', apply: 'تطبيق', content: 'محتوى الواجهة', layout: 'شكل العرض',
  } : {
    builder: 'Storefront builder', sections: 'Store sections', canvas: 'Live preview', settings: 'Store properties', drag: 'Drag to reorder sections', announcement: 'Announcement bar', hero: 'Main hero', trust: 'Store benefits', categories: 'Categories', featured: 'Featured products', products: 'All products', testimonials: 'Testimonials', social: 'Social & contact', visible: 'Visible', hidden: 'Hidden', logo: 'Store logo', banner: 'Store banner', uploadError: 'Could not upload image', unsaved: 'You have unsaved changes', mobile: 'Mobile', desktop: 'Desktop', sampleCategory: 'New products', remove: 'Remove', upload: 'Drop an image here or click to upload', uploading: 'Uploading image…', open: 'Open', closed: 'Closed', shopNow: 'Shop now', all: 'All', offers: 'Offers', popular: 'Popular', bannerHint: 'Recommended 1600 × 600', templates: 'Ready templates', templatesHint: 'Choose a template, then customize every detail', apply: 'Apply', content: 'Hero content', layout: 'Layout style',
  };
