...
export default async function MerchantReferralsPage() {
  ...
  const { program, code, referrals, rewards, commissions } = await getMerchantReferralDashboard(auth.merchantId);
  ...
  return <div className="mx-auto max-w-6xl space-y-6" dir="rtl">
    <header><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-bold text-[var(--primary)]">نمو متجرِك مع وصلة</p><h1 className="mt-2 text-2xl font-black">إحالة متاجر جديدة</h1><p className="mt-2 text-sm leading-7 text-[var(--muted-foreground)]">شارك رابطك مع أصحاب الأعمال. التسجيل المجاني لا يولد عمولة؛ تستحق 20% من الاشتراك المحصل وفق مدة وشروط البرنامج.</p></div><Link href="/dashboard/verification" className="rounded-xl border border-[var(--primary)] px-4 py-2 text-sm font-bold text-[var(--primary)]">الهوية وبيانات السداد</Link></div></header>
    ...
    {program && <section className="rounded-2xl border bg-[var(--card)] p-6"><h2 className="font-black">شروط البرنامج الحالية</h2><dl className="mt-4 grid gap-4 text-sm md:grid-cols-4"><div><dt className="text-[var(--muted-foreground)]">شرط التأهل</dt><dd className="font-bold">أول اشتراك Pro مدفوع ومطابق</dd></div><div><dt className="text-[var(--muted-foreground)]">العمولة</dt><dd className="font-bold">{program.commissionRate.toString()}% من المبلغ المحصل</dd></div><div><dt className="text-[var(--muted-foreground)]">المدة</dt><dd className="font-bold">{program.commissionMonths} شهراً</dd></div><div><dt className="text-[var(--muted-foreground)]">التعليق</dt><dd className="font-bold">{program.holdDays} يوم</dd></div></dl>{program.terms && <p className="mt-4 whitespace-pre-wrap text-sm leading-7">{program.terms}</p>}</section>}
    ...
    <section><h2 className="mb-3 font-black">عمولات الاشتراكات</h2><div className="space-y-3">{commissions.map(item => <article key={item.id} className="rounded-2xl border bg-[var(--card)] p-5"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-bold">{item.referralRecord.referredMerchant.name}</p><p className="text-xs text-[var(--muted-foreground)]">{item.commissionRate.toString()}% من اشتراك {item.grossAmount.toString()} {item.currency}</p></div><p className="font-black">{item.amount.toString()} {item.currency} · {statuses[item.status] ?? item.status}</p></div></article>)}{!commissions.length && <p className="rounded-2xl border border-dashed p-8 text-center text-[var(--muted-foreground)]">لا توجد عمولات بعد؛ التسجيل المجاني لا يحتسب.</p>}</div></section>
  </div>;
}
