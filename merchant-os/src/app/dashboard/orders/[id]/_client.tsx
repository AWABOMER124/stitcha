...
import {
  acceptMerchantDeliveryQuoteAction,
  requestMerchantDeliveryQuotesAction,
} from "@/modules/delivery-partners/actions";
import { createInvoiceAction } from "@/modules/invoices/actions";
import { editOrderAction } from "@/modules/orders/actions";
...
function StatusTimeline({
  ...
  history: ActiveOrder["statusHistory"];
}) {
      ...
      {[...history].reverse().map((entry, i) => (
        <div key={entry.id} className="flex items-start gap-3">
          <div className="flex flex-col items-center">
            <div
              className={`h-3 w-3 rounded-full border-2 ${i === 0 ? "border-[var(--primary)] bg-[var(--primary)]" : "border-[var(--border)] bg-[var(--card)]"}`}
            />
            {i < history.length - 1 && (
              <div
                className="mt-1 h-full w-px bg-[var(--border)]"
          ...
          <div className="pb-4 min-w-0">
            <p className="text-sm font-semibold text-[var(--foreground)]">
              {statusLabel[entry.status as keyof typeof statusLabel] ??
                entry.status}
            </p>
            {entry.note && (
...
export function OrderDetailClient({
  ...
  const nextLabel =
    ft.nextStatusLabel[status as keyof typeof ft.nextStatusLabel];
  ...
  function handlePaymentReview(decision: "VERIFY" | "REJECT") {
    ...
    setError(null);
    startTransition(async () => {
      const result = await reviewOrderPaymentAction({
          ...
          ? {
              ...
              payment: {
                ...
                manualProof: {
                  ...
                  rejectionReason: decision === "REJECT" ? paymentReason : null,
                },
              },
            }
          : previous,
  ...
  function requestDeliveryQuotes() {
    ...
    startTransition(async () => {
      ...
      if (result.data.length === 0)
        setError(
          locale === "ar"
            ? "لا توجد شركة تغطي هذا الموقع حالياً."
            : "No delivery partner currently covers this location.",
        );
    });
  }
  ...
  function openInvoice() {
    if (order.invoice) {
      router.push(`/dashboard/invoices/${order.invoice.id}`);
      return;
    }
    setError(null);
    startTransition(async () => {
  ...
  function saveOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    startTransition(async () => {
      const result = await editOrderAction(order.id, values);
      if (!result.success) {
        setError(result.error);
        return;
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold ${STATUS_COLORS[status] ?? "bg-gray-100 text-gray-700"}`}
            >
        ...
        <div className="flex items-center gap-2">
          {!isTerminal && (
            <button
              ...
              className="rounded-lg border border-[var(--border)] px-4 py-2 text-sm font-bold"
            >
          ...
          <button
            disabled={isPending}
            onClick={openInvoice}
            className="rounded-lg border border-[var(--border)] px-4 py-2 text-sm font-bold text-[var(--foreground)] disabled:opacity-50"
          >
      ...
      {editing && (
        ...
        >
          ...
          <form
            onSubmit={saveOrder}
            className="relative w-full max-w-xl space-y-4 rounded-3xl bg-[var(--card)] p-6 shadow-2xl"
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                  {locale === "ar"
                    ? "متاح قبل بدء التوصيل، وكل تعديل يُسجل في سجل الطلب."
                    : "Available before delivery starts; edits are audited."}
                </p>
              </div>
            ...
            <EditField
              ...
              label={locale === "ar" ? "عنوان التوصيل" : "Delivery address"}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <EditField
                name="deliveryFee"
                type="number"
                label={locale === "ar" ? "رسوم التوصيل" : "Delivery fee"}
                value={String(Number(order.deliveryFee))}
              />
              <EditField
                name="notes"
                label={locale === "ar" ? "ملاحظة العميل" : "Customer note"}
                value={order.notes ?? ""}
              />
            </div>
            ...
            <button
              disabled={isPending}
              className="w-full rounded-xl bg-[var(--primary)] px-4 py-3 font-bold text-white disabled:opacity-50"
            >
              {isPending
                ? "…"
                : locale === "ar"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-[var(--foreground)]">
                  {t.deliveryMethods[
                    order.deliveryMethod as keyof typeof t.deliveryMethods
            ...
            {order.payment?.manualProof && (
              <div className="border-t border-[var(--border)] pt-3 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[var(--muted-foreground)]">
                    حالة إشعار التحويل
                  </span>
                  <strong>{order.payment.manualProof.status}</strong>
                </div>
                <p className="text-xs text-[var(--muted-foreground)]">
                  {order.payment.manualProof.accountLabel} ·{" "}
                  {order.payment.manualProof.transactionRef}
                </p>
                <a
                  href={`/api/order-payments/${order.payment.id}/proof`}
                  target="_blank"
                  rel="noreferrer"
                  className="block rounded-lg border border-[var(--border)] px-3 py-2 text-center text-sm font-semibold"
                >
                  عرض الإشعار الخاص
                </a>
                {order.payment.manualProof.status === "PENDING" && (
                  <div className="space-y-2">
                    <button
                      disabled={isPending}
                      onClick={() => handlePaymentReview("VERIFY")}
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() => handlePaymentReview("REJECT")}
                ...
                {order.payment.manualProof.rejectionReason && (
          ...
          {order.deliveryMethod !== "PICKUP" && (
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 space-y-3">
              <h2 className="text-sm font-semibold text-[var(--foreground)]">
                {locale === "ar" ? "توصيل وصلة" : "Wasla delivery"}
              </h2>
              {order.platformShipment ? (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">
                      {locale === "ar" ? "الشركة" : "Partner"}
                    </span>
                    <strong>{order.platformShipment.partner.name}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">
                      {locale === "ar" ? "الحالة" : "Status"}
                    </span>
                    <strong>{order.platformShipment.status}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">
                      {locale === "ar" ? "رقم التتبع" : "Tracking code"}
                    </span>
                    <span className="font-mono font-semibold">
                      {order.platformShipment.trackingCode}
                    </span>
                  </div>
                  {order.platformShipment.courier && (
                    <div className="rounded-lg bg-[var(--muted)]/40 p-3">
                      <strong className="block">
                        {order.platformShipment.courier.name}
                      </strong>
                      <a
                        href={`tel:${order.platformShipment.courier.phone}`}
                        className="text-[var(--primary)]"
                      >
                        {order.platformShipment.courier.phone}
                      </a>
                    </div>
                  )}
                  {order.platformShipment.codCollection && (
                    <div className="flex justify-between">
                      <span className="text-[var(--muted-foreground)]">
                        COD
                      </span>
                      <strong>
                        {order.platformShipment.codCollection.status} ·{" "}
                        {Number(
                          order.platformShipment.codCollection.expectedAmount,
                        ).toLocaleString()}{" "}
                        {order.platformShipment.codCollection.currency}
                      </strong>
                    </div>
                  )}
                </div>
                ...
                <p className="rounded-lg bg-amber-50 p-3 text-xs text-amber-800">
                  {locale === "ar"
                    ? "الخدمة جاهزة تقنياً لكنها متوقفة لحين اعتماد شركات التوصيل والأسعار في الإنتاج."
                    : "Technically ready, but disabled until production partners and pricing are approved."}
                </p>
              ) : order.delivery?.lat == null || order.delivery.lng == null ? (
                <p className="rounded-lg bg-amber-50 p-3 text-xs text-amber-800">
                  {locale === "ar"
                    ? "لا يمكن طلب عرض دون موقع تسليم دقيق. اطلب من العميل مشاركة الموقع."
                    : "A precise drop-off location is required. Ask the customer to share it."}
                </p>
              ) : order.paymentMethod !== "CASH" ? (
                <p className="rounded-lg bg-amber-50 p-3 text-xs text-amber-800">
                  {locale === "ar"
                    ? "المرحلة التجريبية للتوصيل تدعم الدفع عند الاستلام فقط."
                    : "The delivery pilot currently supports cash on delivery only."}
                </p>
              ) : (
                <div className="space-y-3">
                  <button
                    ...
                    disabled={isPending}
                    className="w-full rounded-lg bg-[var(--primary)] px-3 py-2 text-sm font-bold text-white disabled:opacity-50"
                  >
                    {isPending
                      ? "..."
                      : locale === "ar"
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <span className="text-xs text-[var(--muted-foreground)]">
                            {quote.etaMinutesMin != null
                              ? `${quote.etaMinutesMin}–${quote.etaMinutesMax ?? quote.etaMinutesMin} ${locale === "ar" ? "دقيقة" : "min"}`
                              : locale === "ar"
                                ? "المدة تؤكدها الشركة"
                                : "ETA confirmed by partner"}
                          </span>
                        </div>
                        <strong>
                          {Number(quote.fee).toLocaleString()} {quote.currency}
                        </strong>
                      </div>
                      <button
                        type="button"
                        onClick={() => acceptDeliveryQuote(quote.id)}
                        disabled={isPending}
                        className="mt-3 w-full rounded-lg border border-[var(--primary)] px-3 py-2 text-xs font-bold text-[var(--primary)] disabled:opacity-50"
                      >
                        {locale === "ar" ? "اختيار هذا العرض" : "Select quote"}
                      </button>
                    </div>
                  ))}
                </div>
  ...
  return (
    <label className="block text-sm font-bold">
      ...
      <input
        ...
        required={["customerName", "customerPhone", "deliveryFee"].includes(
          name,
        )}
        defaultValue={value}
        className="mt-2 w-full rounded-xl border border-[var(--border)] bg-transparent px-3 py-2"
