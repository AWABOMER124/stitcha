    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-[var(--border)] p-16 text-center">
          ...
          <p className="font-semibold text-[var(--foreground)]">لا يوجد عملاء</p>
          <p className="text-sm text-[var(--muted-foreground)] mt-1">ستظهر بيانات العملاء بعد استلام الطلبات</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--muted)]/40">
                {['العميل', 'الشريحة', 'الطلبات', 'الإنفاق', 'آخر طلب', ''].map((h) => (
                  <th key={h} className="py-3 px-4 text-right text-xs font-semibold text-[var(--muted-foreground)]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--border)]">
              {filtered.map((c) => {
                ...
                const lastOrder = c.lastOrderAt
                  ? new Date(c.lastOrderAt).toLocaleDateString('ar', { day: '2-digit', month: 'short' })
                  : '—';
                return (
