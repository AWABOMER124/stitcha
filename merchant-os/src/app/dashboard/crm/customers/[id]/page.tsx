  ...
  return (
    <div dir="rtl" className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-2 text-sm text-[var(--muted-foreground)]">
        ...
        <span>/</span>
        <Link href="/dashboard/crm/customers" className="hover:text-[var(--primary)]">العملاء</Link>
        <span>/</span>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 text-center">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 border-t border-[var(--border)] pt-5 text-center">
            <div>
              ...
              <p className="text-xs text-[var(--muted-foreground)]">طلب</p>
            </div>
            <div>
          ...
          {customer.loyaltyAccount && (
            <div className="mt-4 rounded-lg bg-amber-50 border border-amber-200 p-3">
              <p className="text-xs text-amber-600 mb-0.5">نقاط الولاء</p>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
            <div className="px-5 py-4 border-b border-[var(--border)]">
              <h3 className="font-bold text-[var(--foreground)]">سجل الطلبات</h3>
            </div>
            {orders.length === 0 ? (
              <p className="px-5 py-8 text-center text-sm text-[var(--muted-foreground)]">لا توجد طلبات</p>
            ) : (
                  ...
                  <div key={o.id} className="px-5 py-3.5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        ...
                        {o.status === 'DELIVERED' ? 'تم' : o.status === 'CANCELLED' ? 'ملغي' : 'جارٍ'}
                      </span>
