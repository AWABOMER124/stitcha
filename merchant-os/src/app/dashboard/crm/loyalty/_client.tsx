...
export function LoyaltyClient({
  ...
  const summaryLine1 = t.summaryLine1
    .replace('{a}', String(config.pointsPerOrder))
    .replace('{b}', String(config.pointsPerSDG));
  const summaryLine2 = t.summaryLine2
    .replace('{c}', String(config.redemptionThreshold))
    .replace('{d}', String(config.redemptionValueSDG));
    ...
    <div className="space-y-6">
      <form onSubmit={handleSave} className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-[var(--foreground)]">{t.programSettings}</h3>
            <p className="text-xs text-[var(--muted-foreground)] mt-0.5">{t.programSettingsDesc}</p>
          </div>
        ...
        <div className={`space-y-5 ${!config.isEnabled ? 'opacity-40 pointer-events-none' : ''}`}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1.5">{t.pointsPerOrderLabel}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1.5">{t.pointsPerSDGLabel}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1.5">{t.redemptionThresholdLabel}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1.5">{t.redemptionValueLabel}</label>
          ...
          <div className="rounded-lg bg-[var(--primary)]/5 border border-[var(--primary)]/20 p-4 text-sm">
            <p className="font-semibold text-[var(--primary)] mb-1">{t.summaryTitle}</p>
            <p className="text-[var(--foreground)]">{summaryLine1}</p>
            <p className="text-[var(--foreground)]">{summaryLine2}</p>
          </div>
        </div>
            ...
            {leaderboard.map((acc: any, i: number) => (
              <div key={acc.id} className="px-5 py-3.5 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-sm text-[var(--foreground)]">{acc.customer?.name ?? t.unknownCustomer}</p>
