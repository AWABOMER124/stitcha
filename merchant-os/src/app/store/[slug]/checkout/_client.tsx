    ...
    <div dir={dir} className="min-h-screen bg-stone-50">
      ...
      <header className="bg-white border-b border-stone-200 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="font-bold text-stone-900">{t.checkoutTitle}</h1>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-4 space-y-3">
          <h2 className="font-bold text-stone-900">{t.orderSummary}</h2>
          {cart.map((item, i) => (
        ...
        {/* Contact */}
        <div className="bg-white rounded-2xl border border-stone-100 p-4 space-y-3">
          <h2 className="font-bold text-stone-900">{t.yourDetails}</h2>
          <div>
            <label className="text-sm text-stone-600 mb-1 block">{t.nameLabel}</label>
            <input value={name} onChange={e => setName(e.target.value)} required placeholder={t.fullNamePlaceholder} className="w-full border border-stone-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[var(--sp)] focus:ring-2 focus:ring-[color-mix(in_srgb,var(--sp)_15%,transparent)]" />
          </div>
          <div>
        ...
        {/* Delivery Method */}
        <div className="bg-white rounded-2xl border border-stone-100 p-4 space-y-3">
          <h2 className="font-bold text-stone-900">{t.deliveryMethodTitle}</h2>
          <div className="grid grid-cols-2 gap-3">
            {settings?.pickupEnabled !== false && (
              <button type="button" onClick={() => setMethod('PICKUP')} className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 text-sm font-medium transition-all ${method === 'PICKUP' ? 'border-[var(--sp)] bg-[color-mix(in_srgb,var(--sp)_8%,white)]' : 'border-stone-100 text-stone-500 hover:border-stone-200'}`} style={method === 'PICKUP' ? { color: primary } : {}}>
                <span className="text-2xl">🏪</span> {t.pickupOption}
              </button>
            )}
            {settings?.deliveryEnabled !== false && (
              <button type="button" onClick={() => setMethod('MERCHANT_DELIVERY')} className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 text-sm font-medium transition-all ${method === 'MERCHANT_DELIVERY' ? 'border-[var(--sp)] bg-[color-mix(in_srgb,var(--sp)_8%,white)]' : 'border-stone-100 text-stone-500 hover:border-stone-200'}`} style={method === 'MERCHANT_DELIVERY' ? { color: primary } : {}}>
                <span className="text-2xl">🚚</span> {t.deliveryOption}
              </button>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-4">
          <label className="font-bold text-stone-900 block mb-2">{t.notesForMerchant}</label>
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder={t.notesPlaceholder} className="w-full border border-stone-200 rounded-xl px-3 py-2 text-sm outline-none resize-none" />
        </div>
